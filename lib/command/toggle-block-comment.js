function toggleBlockComment(cmc) {
  ...
  if (isInComment(selectedArea, lines)) {
    ...
    if (
      ...
      cmc.setSelection(
        selectedArea.from.line + (begin === selectedArea.to ? 2 : 0),
        0,
        selectedArea.to.line + (end === selectedArea.to ? 2 : 0),
        0
      );
    } else {
      ...
      let ch2 =
        ...
        (selectedArea.from.line === selectedArea.to.line
          ...
          : 0);
      if (isSelected) {
        ch1 += commentBegin.length * (isSelected && end === selectedArea.from ? 0 : -1);
        ch2 += commentEnd.length * (isSelected && end === selectedArea.from ? 0 : 1);
      }
