...
function hasBeginComment(lines, cursor) {
    for (let i = cursor.line; 0 <= i; i--) {
        let line;
        if (i === cursor.line) {
            line = lines[i].substring(0, cursor.ch);
        } else {
            line = lines[i];
        }
...
function hasEndComment(lines, cursor) {
    for (let i = cursor.line; i < lines.length; i++) {
        let line;
        if (i === cursor.line) {
            line = lines[i].substring(cursor.ch);
        } else {
            line = lines[i];
        }

        const commentBeginIndex = line.indexOf(commentBegin);
        const commentEndIndex = line.indexOf(commentEnd);
        if (commentBeginIndex !== -1 && commentBeginIndex < commentEndIndex) {
            return false;
...
module.exports.execute = (cmc) => {
    ...
    if (isInComment(selectedArea, lines)) {
        ...
        if (start.line === last.line) {
            ...
            lines[start.line] =  line.slice(0, start.ch) + line.slice(start.ch + commentBegin.length, last.ch) + line.slice(last.ch + commentEnd.length);
        } else {
            if (lines[last.line] === commentEnd) {
                lines.splice(last.line, 1);
            } else {
                const line = lines[last.line];
                lines[last.line] = line.slice(0, last.ch) + line.slice(last.ch + commentEnd.length);
            }
            if (lines[start.line] === commentBegin) {
                lines.splice(start.line, 1);
            } else {
                const line = lines[start.line];
                lines[start.line] = line.slice(0, start.ch) + line.slice(start.ch + commentBegin.length);
            }
        }
        cmc.setLines(lines);
    } else {
        if (selectedArea.from.line !== selectedArea.to.line && selectedArea.from.ch === 0 && selectedArea.to.ch === 0) {
            ...
            let ch2 = selectedArea.to.ch + (selectedArea.from.line === selectedArea.to.line ? commentBegin.length : 0);
            if (isSelected) {
